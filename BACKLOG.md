...
- [ ] Metronome: click on the beat (accent on the downbeat) while recording and playing, toggle in the transport, so takes land on the 120 BPM grid the score and MIDI export assume.
- [ ] Count-in: pressing Record counts down four beats (shown in the transport, clicked once the metronome exists) before anything is captured, so the first note lands on the downbeat.
