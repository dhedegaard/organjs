...
- `src/sequence/` — framework-free recording: `sequence.ts` (zod `Sequence`: notes as start/duration intervals, settings changes as raw CCs), `recorder.ts` (live events → sequence), `player.ts` (steps a sequence against an injected clock; off before CC before on at equal times), `edit.ts` (pure piano-roll edits, snap grid), `midiFile.ts` (SMF format 0 writer / format 0–2 reader, 480 PPQ at 120 BPM, tempo map honoured), `notation.ts` (sequence → 4/4 grand-staff events at 120 BPM: sixteenth grid, one voice per staff, overlaps cut into tied chord segments, sharps-only spelling with per-bar accidentals), `storage.ts` (named recordings in local storage), `time.ts`.
...
- `src/components/` — `Keyboard` (pointer glide via `elementFromPoint`, keys carry `data-midi`), `Drawbar`/`Drawbars` (custom `role=slider`, pull down = louder), `Tab` (rocker switch, optional `states` labels), `CopyLink`, `Transport` (record/play/loop/tempo), `PianoRoll` (SVG editor; pointer maths from `getBoundingClientRect`, tests pin it with a spy; Roll/Score toggle in its toolbar), `Score` (read-only grand staff, proportional to time, click to seek; glyphs are SMuFL codepoints in a 6 KB Bravura subset at `src/assets/`, OFL licence alongside), `Library` (save/load, .mid import/export).
- `src/index.css` — single stylesheet, design tokens on `:root`. Light only (`color-scheme: light`, no dark mode): daylit room, blond oak cabinet, parchment panels, walnut ink for text and notation, ivory/ebony keys, Hammond cap colours (brown 16'/5⅓', black mutations, white unisons). Brass is a fill colour; use `--brass-ink` where it carries text or lines. One typeface: Instrument Sans.
...
- Module-level constants shared by two components that import each other (e.g. `RULER_HEIGHT`) live in `components/timeline.ts`; a cycle through `PianoRoll` ↔ `Score` throws "cannot access before initialization" at load.
- To add a music glyph, re-subset Bravura with `pyftsubset Bravura.woff2 --unicodes=... --flavor=woff2` (fontTools in a scratchpad venv) and add its codepoint to `GLYPH` in `Score.tsx`; the staff space is 8 px and glyphs are drawn at a 32 px font size (4 spaces per em).
